...
const EventCard = ({ data, heading }) => {
    ...
    const renderElements = (element, index) => {
        const renderedElement = (
            <div id="core-members" className="flex flex-col items-center justify-between text-white mx-auto text-center bg-transparent border-2 border-white border-r-0 border-t-0 w-full p-4 gap-y-4 min-h-64 transition-all hover:scale-105 duration-150 ease-linear">
                <h1 className="text-2xl font-semibold">{element.title}</h1>
                <p className="text-lg italic text-white">{element.description}</p>
    ...
    return (
        <div>
            <h1 className="w-full ml-0 lg:ml-8 mx-auto text-center lg:text-start text-white text-4xl font-bold mb-8">{heading}</h1>
